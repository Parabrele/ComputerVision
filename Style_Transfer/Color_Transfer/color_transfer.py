...
def stat_color_transfer(image, style, Lab=False, wct=False):
    if Lab:
        image = images.rgb_to_lab(image)
        style = images.rgb_to_lab(style)

    if wct:
        image = images.wct(image, style)
    else:
        image = images.stat_transfer(image, style)

    if Lab:
        clamp1 = torch.clamp(image[:, 0], 0, 100)
        clamp2 = torch.clamp(image[:, 1:], -127, 128)
        image = torch.cat((clamp1.unsqueeze(1), clamp2), dim=1)
...
def kmeans(image, k):
    ...
    kstack = torch.zeros((k, 3)).to(image.device)
    kstack[0] = k0
...
def kmeans_color_transfer(image, style, k=3, Lab=False, wct=False):
    ...
    ###
    # TODO : do this and advanced stat transfer

    if wct:
        print('WCT not implemented yet')
        return
        ...
        imlab = flatten_image(image)
        stlab = flatten_image(style)
