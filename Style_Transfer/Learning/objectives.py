...
def match_wct(model, layer_names, content, style, alpha=0.9, loss_fct=mean_L1):
    """
    compute features of content and style, perform wct and save it as the target.
    """
    
    style_features = get_stream(model, layer_names, style)
    content_features = get_stream(model, layer_names, content().detach())

    wcted = []
    for i, c_f in enumerate(content_features):
        wcted.append(images.wct(c_f, style_features[i], alpha=alpha))

    layer_hook = [get_hook(model, layer_name) for layer_name in layer_names]

    def obj_fct(model_output):
        loss = 0
        for i, hook in enumerate(layer_hook):
            features = hook.output
            wcted_activation = wcted[i]

            loss += loss_fct(features, wcted_activation)
